...
def create_access_token(data: dict):
    """
    Создаёт JWT access_token с payload (sub, role, id, exp).
    """
    to_encode = data.copy()
...
async def get_current_admin_or_buyer(current_user: UserModel = Depends(get_current_user)):
    """
    Проверяет, что пользователь имеет роль 'admin' или 'buyer'.
    """
    if current_user.role not in ("admin", "buyer"):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only admin or buyer can perform this action")
    return current_user
